...
public:
	...
	void validate()
	{
		...
		{
			cout << "Invalid Board size: " << boardSize.width << " " << boardSize.height << endl;
		...
		if (nrFrames <= 0)
		{
			cout << "Invalid number of frames " << nrFrames << endl;
		...
		{
			if ((input[0] >= '0' && input[0] <= '9') || newCamera == true)
			{
		...
		{
			cout << "Input does not exist: " << input;
...
void calibrateAndSave() {
	...
	if (s.inputType == Settings::IMAGE_LIST && s.showUndistorsed)
	{
		...
		for (size_t i = 0; i < s.imageList.size(); i++)
		{
			...
			cv::line(rview, Point(10, 20), Point(40, 60), cv::Scalar(0, 255, 200), 1, 8, 0);
	...
	vector<vector<Point2f> > imagePoints)
{
	...
	bool ok = runCalibration(s, imageSize, cameraMatrix, distCoeffs, imagePoints, rvecs, tvecs, reprojErrs,
		totalAvgErr);
	cout << "Camera mat: " << cameraMatrix << endl;
